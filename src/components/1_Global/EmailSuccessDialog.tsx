...
const EmailSuccessDialog = ({ isDialogOpen, onOpenChange, message }: MyDialog) => {
    return (
        <div>
            <Dialog.Root open={isDialogOpen} onOpenChange={onOpenChange}>
                ...
                <Dialog.Portal>
                    ...
                    <Dialog.Overlay className="fixed inset-0 bg-black/50 flex items-center justify-center z-[1000]" />
                    ...
                    <Dialog.Content className="fixed z-90 left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 bg-gray-900 p-8 rounded-lg shadow-xl text-center max-w-sm w-[90%] animate-in fade-in zoom-in-95 data-[state=closed]:animate-out data-[state=closed]:fade-out data-[state=closed]:zoom-out-95 duration-200">
                        ...
                        <Dialog.Description className="text-lg text-white mb-6">
                            {message}
                        </Dialog.Description>
                        ...
                        {/*Close Buttons*/}
                        <Dialog.Close>
                            <button className='rounded-lg block w-max mx-auto text-sm px-4 py-2 bg-white text-black cursor-pointer
